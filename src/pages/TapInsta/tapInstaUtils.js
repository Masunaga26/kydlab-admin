...
export const normalizePublicCode=v=>String(v??"")
  .toUpperCase()
  .replace(/[^A-Z0-9]/g,"")
  .slice(0,6);

export const normalizeActivationCode=v=>String(v??"")
  .toUpperCase()
  .replace(/[^A-Z]/g,"")
  .slice(0,4);
...
export function instagramUrl(v){
  const u=normalizeInstagramUsername(v);
  if(!validInstagram(u))throw new Error("Instagram inválido.");
...
export const formatDateTime=v=>v
